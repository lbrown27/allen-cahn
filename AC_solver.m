...
t_vec = 0;
integral_thickness_water_side = [0];
...
while physical_time < .1
    ...
    if count > 1
        c_new = c_step(c_n, T_n,u_n,c_old,eta_new, eta_old,rho_n, x_coll,pc);
    else
        c_new = c_step_fe(c_n, T_n,u_n,eta_n,rho_n, x_coll,pc);
    end
    ...
    T_new = solve_temp_CN(rho_cp_n, rho_cp_new,u_new,u_n,k_new, k_n,T_n, c_new,eta_new,rho_new,pc.wall_T,x_coll,pc);

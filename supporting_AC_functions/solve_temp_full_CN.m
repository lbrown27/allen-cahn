...
h_prime = h_prime + (h_prime == 0).*(30 .*(c_new-1).^2.*c_new.^2);
    ...
    A_temp = sparse(diag(rho_cp_new) - .5 * pc.dt * E_new+.5*diag(pc.Mc*pc.L^2*pc.rho_water^2*h_prime/pc.T_M));
    ...
    D = rho_cp_n .* T_n;
F = -.5*rhs_ac_arezoo_for_implicit_T(c_new,u_new,T_n,eta_new,rho_new, pc,0);
    Visc = zeros(pc.N + 2,1);
    for i = 2: pc.N + 1
